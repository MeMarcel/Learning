...
std::vector<ProjectileData> initializeProjectileData()
{
	...
	data[Projectile::Missile].speed = 700.f;
...
std::vector<PickupData> initializePickupData(){
    ...
    data[Pickup::FireSpread].action = std::bind(&Aircraft::increaseFireSpread, _1,1);

       data[Pickup::FireRate].texture = Textures::FireRate;
    data[Pickup::FireRate].action = std::bind(&Aircraft::increaseFireRate, _1,1);
}
